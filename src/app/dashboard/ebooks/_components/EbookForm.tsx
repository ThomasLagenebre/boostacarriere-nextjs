"use client";
import { createEbook } from '@/app/_data/createEbook';
...
function EbookForm() {
    ...
    const [request, setRequest] = useState({
        ...
        promotion: 0,
        promotionTime: 0,
        currentProblems: [] as ProblemObject[],
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
          const response = await createEbook(request);       
          if (response.ok) {
            ...
            toast.error(errorData.message || 'Erreur lors de la création de l\'ebook');
          }
      } catch (error: any) {
