  ...
  return (
        <div className='my-4'>
            ...
            {(type === 'number' || type === 'text') && (
                <input type={type} id={id} className={`${inputClassName} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`} placeholder={placeholder} required={required} value={currentValue} disabled={disabled} onChange={(e) => handleChangeValue(e.target.value)}/>
            )}
            {type === 'textarea' && (
                <textarea id={id} rows={rows} className={`${inputClassName} block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`} placeholder={placeholder} required={required} value={currentValue} disabled={disabled} onChange={(e) => handleChangeValue(e.target.value)}></textarea>
            )}
